...
namespace Radiant.WebScraper.Scrapers.Manual
{
    public static class ManualAutomation
    {
        ...
        public static void KeyboardExecute(Keycode aKeycode, int? aDelay = null)
        {
            ...
            {
                Delay = aDelay,
                KeyStrokeCodes = new[] { aKeycode }
            });

            if (!_Result.Success)
                LoggingManager.LogToFile($"Failed to execute keyboard keycode [{aKeycode}].");
        }
