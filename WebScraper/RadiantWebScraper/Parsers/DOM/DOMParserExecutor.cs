...
namespace Radiant.WebScraper.RadiantWebScraper.Parsers.DOM
{
    public static class DOMParserExecutor
    {
        ...
        public static string Execute(string aUrl, string aDOM, DOMParserItem aParserItem)
        {
            ...
            if (_Matches.Count <= 0)
            {
                LoggingManager.LogToFile("95276771-4C9B-47FD-A2ED-1F3F2E707F9D", $"DOM parser couldn't match content. ParserItem config - UID [{aParserItem.UID}]. Regex = [{aParserItem.RegexPattern}].");
                return null;
            }
